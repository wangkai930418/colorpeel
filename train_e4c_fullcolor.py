...
from colornet_utils import create_image_with_shapes, _compute_cosine, ColorNet_Embed, ColorEmbed, optim_init_colornet_list
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="models/colorpeel_e4c_10000steps_fullcolor",
    ...
    parser.add_argument(
        "--color_name_list", type=str, 
        default="maroon+olive+gray+teal+navy", 
    )
...
def main(args):
    ...
    train_dataset = CustomDiffusionDataset_FullColor(
        tokenizer=tokenizer,
    ...
    optimizer = optimizer_class(
        itertools.chain(text_encoder.get_input_embeddings().parameters(), \
                            custom_diffusion_layers.parameters(), \
                                color_encoder.parameters(),
                                    color_embedder.parameters(),
                                    )
        if args.modifier_token is not None
    ...
    if args.modifier_token is not None:
        custom_diffusion_layers, text_encoder, optimizer, train_dataloader, lr_scheduler, color_encoder, color_embedder = accelerator.prepare(
            custom_diffusion_layers, text_encoder, optimizer, train_dataloader, lr_scheduler, color_encoder, color_embedder
        )
    else:
        custom_diffusion_layers, optimizer, train_dataloader, lr_scheduler, color_encoder, color_embedder = accelerator.prepare(
            custom_diffusion_layers, optimizer, train_dataloader, lr_scheduler, color_encoder, color_embedder
        )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder):
                ...
                color_embed = color_encoder(color_embed_pre)
    ...
    if accelerator.is_main_process:
        ...
        torch.save(color_embedder.cpu().state_dict(), f"{args.output_dir}/color_embedder.pth")
        # Final inference
