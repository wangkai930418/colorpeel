...
### NOTE: modify this class for color encoder
class CustomDiffusionDataset_ColorList(Dataset):
    def __init__(
        self,
        tokenizer,
        size=512,
...
class CustomDiffusionDataset_FullColor(Dataset):
    ...
    def __getitem__(self, index):
        ...
        color_fill_ = random.sample(list(range(self.num_labels)), 3)
        color_fill = [item * int(256.0/self.num_labels) for item in color_fill_] ###
